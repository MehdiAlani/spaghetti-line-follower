...
void map_section_pid(){
  ...
  IR_KD = 0.015;
  IR_KI = 0.05;
  while(distance - prev_distance < 200){
    
    ir_error = (ir_array[5] + ir_array[6] + ir_array[7] + ir_array[8]) - (ir_array[3] + ir_array[2] + ir_array[1] + ir_array[0]);
...
#define SECTION0_DISTANCE_0 15.0
#define SECTION0_DISTANCE_1 40.0
void map_section0(){
  ...
  while(distance - prev_distance < SECTION0_DISTANCE_0){
  ...
  prev_distance = distance;
  while((distance - prev_distance) < SECTION0_DISTANCE_1){
...
#define SECTION1_DISTANCE_1 5.0

#define SECTION1_ANG_0 60
#define SECTION1_ANG_1 -60
...
void map_section1(){
  ...
  while(imu_z_raw - prev_imu < SECTION1_ANG_0){
    ...
    if(ir_array[6] < BLACK_THRESH_LEFT) {
      ir_error += (ir_array[6] - 4095) * 2;
    }
    if(ir_array[7] < BLACK_THRESH_LEFT) {
      ir_error += (ir_array[7] - 4095) * 2;
    }
    if(ir_array[8] < BLACK_THRESH_LEFT) {
      ir_error += (ir_array[8] - 4095) * 2; 
    }
...
#define MAP_SECTION2_DISTANCE_2 10.0
#define MAP_SECTION2_DISTANCE_3 3.0
#define MAP_SECTION2_DISTANCE_4 5.0

#define SECTION2_ANG_0 -50
#define SECTION2_ANG_1 70
#define SECTION2_ANG_2 -55
#define SECTION2_ANG_3 40
#define SECTION2_ANG_4 -30
...
#define MAP_SECTION3_DISTANCE_0 60.0
...
#define SECTION4_DISTANCE_0 20.0
void map_section4(){
  float prev_imu;
  float prev_distance;
